...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header("Location: add_media_coverage.php");
    exit;
}
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO media_coverage (type, reference_id) VALUES (?, ?)");
    $stmt->bind_param("si", $type, $reference_id);

    if (!$stmt->execute()) {
        throw new Exception("Error adding media coverage: " . $stmt->error);
    }

    $media_coverage_id = $stmt->insert_id;
    ...
    if (!empty($_FILES['media']['name'][0])) {
        ...
        foreach ($_FILES['media']['tmp_name'] as $key => $tmp_name) {
            ...
            $stmt = $conn->prepare("INSERT INTO media_media_coverage (media_coverage_id, media_url, media_type) VALUES (?, ?, ?)");
            $stmt->bind_param("iss", $media_coverage_id, $media_url, $media_type);
    ...
    header("Location: success.php?message=" . urlencode("Media coverage added successfully!"));
    exit;
    ...
    error_log("Error in save_media_coverage.php: " . $e->getMessage());
