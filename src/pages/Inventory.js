...
const TransactionsPage = () => {
  ...
  const fetchTransactionStatus = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/transaction/get-transactions-status`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
      ...
      const data = await response.json();
      const transactionsByItem = data.transactions_by_item ?? {};
      const statusByItem = {};

      Object.keys(transactionsByItem).forEach(itemName => {
        transactionsByItem[itemName].forEach(transaction => {
          const itemId = transaction.item_id ?? 'Not available';

          if (!statusByItem[itemId]) {
            statusByItem[itemId] = [];
          }

          statusByItem[itemId].push({
            transaction_type: transaction.transaction_type ?? 'Not available',
            amount_modified: transaction.amount_modified ?? 'Not available',
            previous_amount_available: transaction.previous_amount_available ?? 'Not available',
            previous_amount_on_hold: transaction.previous_amount_on_hold ?? 'Not available',
            present_amount_available: transaction.present_amount_available ?? 'Not available',
            present_amount_on_hold: transaction.present_amount_on_hold ?? 'Not available',
            uom: !['Not changed', 'Not available'].includes(transaction.uom) ? transaction.uom : '',
          });
        });
      });

      setTransactionStatus(statusByItem);
    } catch (error) {
      console.error('Error fetching transaction status:', error);
      setTimeout(() => {
        toast.error('Failed to fetch transaction status data', {
          position: 'top-center',
        });
      }, 300);
    } finally {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: tooltipItem => {
            ...
            } else if (datasetLabel === 'Allocated') {
              return `Allocated: ${value} ${uom}`;
      ...
      datalabels: {
        ...
        formatter: (value, context) => {
          ...
          if (datasetLabel === 'Available' || datasetLabel === 'Allocated') {
            return `${value} ${uom}`;
          }
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
          ...
          <Table className="min-w-full">
            <TableHead className="bg-gray-300">
              <TableRow>
                ...
                <TableCell align="center" className="font-bold text-lg text-gray-700 px-6 py-4">
                  <strong>Allocated for Event</strong>
                </TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {inventory
                .sort((a, b) => (a.item_name === 'Money' ? 1 : b.item_name === 'Money' ? -1 : 0))
                .map(item => (
                  <React.Fragment key={item.item_id}>
                    <TableRow
                      className={`transition-colors duration-150 ${
                        item.item_name === 'Money'
                    ...
                    <TableRow>
                      <TableCell colSpan={4} style={{ padding: 0 }}>
                        <Collapse in={expandedItems[item.item_id]} timeout="auto" unmountOnExit>
                            ...
                            <Table size="small">
                              <TableHead className="bg-gray-200">
                                <TableRow>
                                  ...
                                  <TableCell align="center" className="font-semibold">
                                    <strong>Previous Available</strong>
                                  </TableCell>
                                  <TableCell align="center" className="font-semibold">
                                    <strong>Previous Allocated</strong>
                                  </TableCell>
                                  <TableCell align="center" className="font-semibold">
                                    <strong>Present Available</strong>
                                  </TableCell>
                                  <TableCell align="center" className="font-semibold">
                                    <strong>Present Allocated</strong>
                                  </TableCell>
                                </TableRow>
                              </TableHead>
                              <TableBody>
                                {transactionStatus[item.item_id] &&
                                transactionStatus[item.item_id].length > 0 ? (
                                  transactionStatus[item.item_id].map((transaction, index) => (
                                    <TableRow key={index} className="hover:bg-gray-100">
                                      <TableCell align="center">
                                        {transaction.transaction_type === 'HOLD'
                                          ? 'ALLOCATED'
                                          : transaction.transaction_type === 'UNHOLD'
                                            ? 'UNALLOCATED'
                                            : transaction.transaction_type}
                                      </TableCell>
                                      <TableCell align="center">
                                        {transaction.amount_modified}{' '}
                                        {!['Not available', 'Not changed'].includes(
                                          transaction.amount_modified,
                                        )
                                          ? transaction.uom
                                          : ''}
                                      ...
                                      <TableCell align="center">
                                        {transaction.previous_amount_available}{' '}
                                        {!['Not available', 'Not changed'].includes(
                                          transaction.previous_amount_available,
                                        )
                                          ? transaction.uom
                                      ...
                                      <TableCell align="center">
                                        {transaction.previous_amount_on_hold}{' '}
                                        {!['Not available', 'Not changed'].includes(
                                          transaction.previous_amount_on_hold,
                                        )
                                          ? transaction.uom
                                      ...
                                      <TableCell align="center">
                                        {transaction.present_amount_available}{' '}
                                        {!['Not available', 'Not changed'].includes(
                                          transaction.present_amount_available,
                                        )
                                          ? transaction.uom
                                      ...
                                      <TableCell align="center">
                                        {transaction.present_amount_on_hold}{' '}
                                        {!['Not available', 'Not changed'].includes(
                                          transaction.present_amount_on_hold,
                                        )
                                          ? transaction.uom
        ...
        <div className="flex justify-center items-center mb-6">
          ...
          <Dialog
            ...
            onClose={clearStockReportStates}
            PaperProps={{
              style: {
                borderRadius: '16px',
                padding: '16px',
          ...
          >
            ...
            >
              <Button onClick={clearStockReportStates} variant="contained" color="error">
                Cancel
              </Button>
