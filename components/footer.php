<!-- Footer Section -->
<div class="container mt-2 padding footer">
    <div class="row-footer d-flex">
        <div class="footer-logo">
            <div class="nav-brand">
                <a href="index.php"><i class="fas fa-hands-helping"></i> <span>NGO</span></a>
            </div>
            <ul class="d-flex">
                <li>
                    <a href="#"><i class="fab fa-facebook-f"></i></a>
                </li>
                <li>
                    <a href="#"><i class="fab fa-instagram"></i></a>
                </li>
                <li>
                    <a href="#"><i class="fab fa-twitter"></i></a>
                </li>
            </ul>
        </div>
        <div class="footer-item">
            <h2>Our Pagges</h2>
            <ul>
                <li>
                    <a href="index.php">Home</a>
                </li>
                <li>
                    <a href="#about">About</a>
                </li>
                <li>
                    <a href="#volunteers">Our Volunteer</a>
                </li>
                <li>
                    <a href="#contact">Contact</a>
                </li>
                <li>
                    <a href="donate.php">Donate Now</a>
                </li>
            </ul>
        </div>
        <div class="footer-contact">
            <h2>Contact Now</h2>
            <ul>
                <li>
                    <i class="fas fa-map-marker-alt"></i> <span>12, Pune, Maharastra, India</span>
                </li>
                <li>
                    <i class="fas fa-phone-volume"></i> <span>+91 0000 00000</span>
                </li>
                <li>
                    <i class="far fa-envelope"></i> <span>example@info.com</span>
                </li>
            </ul>
        </div>
    </div>
    <div class="row-developer">
        <p>Designed By</p>
        <a href="#"><span>VedicXcel Innovations</span> <br>www.vedicxcel.com</a>
        <ul class="d-flex">
            <li>
                <a target="_blank" href="https://pruthviraj-rajput-portfolio.rf.gd/"><i
                        class="far fa-handshake"></i></a>
            </li>
            <li>
                <a target="_blank" href="https://github.com/pruthvi7384"><i class="fab fa-github"></i></a>
            </li>
            <li>
                <a target="_blank" href="https://www.linkedin.com/in/pruthviraj-rajput-131ba61b4/"><i
                        class="fab fa-linkedin-in"></i></a>
            </li>
        </ul>
    </div>
</div>
<!--X- Footer Section -X-->

<!-- JavaScript file -->
<script src="js/script.js"></script>
<!--X- JavaScript file -X-->
</body>

</html>